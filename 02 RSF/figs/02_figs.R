...
ggsave(here::here("02 RSF", "figs/g-space_temp.png"), plot = temp_plot, 
       width = 400, height = 300, units = "px", dpi = 110)
ggsave(here::here("02 RSF", "figs/g-space_precip.png"), plot = precip_plot, 
       width = 400, height = 300, units = "px", dpi = 110)
...
ggsave(here::here("02 RSF", "figs/e-space.png"), plot = e_plot, 
...
ggsave(here::here("02 RSF", "figs/habitat.png"), plot = hab_plot, 
...
ggsave(here::here("02 RSF", "figs/avail.png"), plot = avail_fig, 
       width = 800, height = 300, units = "px", dpi = 120)

# Use ----
(temp_u <- hab_pts %>% 
   ggplot(aes(x = temp)) +
   geom_density(linewidth = 1.2, bounds = range(temp) + c(-1, 1)) +
   xlab("Temperature (\u00b0C)") +
...
(precip_u <- hab_pts %>% 
    ggplot(aes(x = precip)) +
    geom_density(linewidth = 1.2, bounds = range(precip) + c(-1, 1)) +
    xlab("Precipitation (mm)") +
    ylab(expression(f[u](x))) +
    coord_cartesian(xlim = range(precip),
                    ylim = c(0, 0.1)) +
...
ggsave(here::here("02 RSF", "figs/use.png"), plot = use_fig, 
       width = 800, height = 300, units = "px", dpi = 120)
...
(precip_w <- bind_rows("a" = g_dat,
    ...
    geom_density(size = 1.2, bounds = range(precip) + c(-1, 1)) +
    xlab("Precipitation (mm)") +
    ylab(expression(f(x))) +
    coord_cartesian(xlim = range(precip),
                    ylim = c(0, 0.1)) +
...
ggsave(here::here("02 RSF", "figs/select.png"), plot = sel_fig, 
       width = 900, height = 280, units = "px", dpi = 120)

# Simple example pie chart ----
{
  agg_png(here::here("02 RSF", "figs/simple_pie.png"), width = 600, height = 400, units = "px",
          res = 150)
  par(mar = rep(0, 4))
